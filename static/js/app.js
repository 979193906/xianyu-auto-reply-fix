...
const LOCAL_VERSION_HISTORY = {
    version: 'v2.1.0',
    intro: '闲鱼自动回复管理系统 — 基于 GuDong2003/xianyu-auto-reply-fix 二次开发',
    versionHistory: [
        {
            version: 'v2.1.0',
            date: '2026-04-13',
            updates: [
                '【新功能】右侧面板重构为三Tab布局：快捷回复 / 商品回复 / 更多',
                '【新功能】快捷回复管理支持分组、触发关键词、点击直接发送',
                '【新功能】聊天界面支持粘贴(Ctrl+V)和拖拽发送图片',
                '【新功能】新消息到达时显示红色未读角标并播放提示音',
                '【优化】快捷回复添加/编辑改为内联表单，交互更美观',
                '【优化】Tab栏和快捷回复列表样式全面美化',
                '【修复】配置热更新清单，支持从自有 GitHub 仓库在线更新'
            ]
        },
        {
            version: 'v2.0.0',
            date: '2026-04-12',
            updates: [
                '【新功能】自建在线客服聊天UI，替代跨域iframe方案',
                '【新功能】三栏布局：账号列表 / 会话列表 / 聊天区',
                '【新功能】基于 WebSocket + SSE 的实时消息收发',
                '【新功能】商品回复设置集成到右侧面板',
                '【新功能】支持一键复用关键词到其他商品',
                '【优化】聊天气泡样式优化，发送方黄色/接收方白色',
                '【优化】会话列表显示买家名称而非ID',
                '【修复】消息发送后不再重复显示（SSE去重）',
                '【修复】Docker热更新环境变量配置支持自有仓库'
            ]
        }
    ]
};
...
function showQuickReplyForm(editId, oldContent, oldGroup, oldTrigger) {
    const isEdit = !!editId;
    const area = document.getElementById('qrFormArea');
    if (!area) return;
    area.innerHTML = `
      <div class="qr-form-card">
        <div class="mb-2">
          <label class="form-label">回复内容 <span class="text-danger">*</span></label>
          <textarea class="form-control" id="qrFormContent" rows="2" placeholder="输入快捷回复内容...">${isEdit ? escapeHtml(oldContent) : ''}</textarea>
        </div>
        <div class="d-flex gap-2 mb-2">
          <div style="flex:1;">
            <label class="form-label">分组</label>
            <input type="text" class="form-control" id="qrFormGroup" value="${isEdit ? escapeHtml(oldGroup) : '默认'}" placeholder="默认">
          </div>
          <div style="flex:1;">
            <label class="form-label">触发关键词</label>
            <input type="text" class="form-control" id="qrFormTrigger" value="${isEdit ? escapeHtml(oldTrigger || '') : ''}" placeholder="可选">
          </div>
        </div>
        <div class="qr-form-actions">
          <button class="btn qr-btn-save" onclick="submitQuickReplyForm(${isEdit ? editId : 'null'})">${isEdit ? '保存修改' : '确认添加'}</button>
          <button class="btn qr-btn-cancel" onclick="hideQuickReplyForm()">取消</button>
        </div>
      </div>`;
    setTimeout(() => { const el = document.getElementById('qrFormContent'); if (el) el.focus(); }, 50);
}

function hideQuickReplyForm() {
    const area = document.getElementById('qrFormArea');
    if (area) area.innerHTML = '';
}

async function submitQuickReplyForm(editId) {
    const content = (document.getElementById('qrFormContent')?.value || '').trim();
    const group = (document.getElementById('qrFormGroup')?.value || '').trim() || '默认';
    const trigger = (document.getElementById('qrFormTrigger')?.value || '').trim();
    if (!content) { showToast('请输入回复内容', 'warning'); return; }
    try {
        const isEdit = editId !== null && editId !== undefined;
        const url = isEdit ? `${apiBase}/api/chat/quick-replies/${editId}` : `${apiBase}/api/chat/quick-replies`;
        const resp = await fetch(url, {
            method: isEdit ? 'PUT' : 'POST',
            headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ content, group_name: group, trigger_keyword: trigger })
        });
        const result = await resp.json();
        if (result.success) {
            ...
            showToast(result.detail || (isEdit ? '修改失败' : '添加失败'), 'danger');
        }
    } catch (e) {
        showToast(isEdit ? '修改失败' : '添加失败', 'danger');
    }
}
...
function editQuickReply(id, oldContent, oldGroup, oldTrigger) {
    showQuickReplyForm(id, oldContent, oldGroup, oldTrigger);
}
